...
cmdparser.add_option("-t", "--test", action="store", dest="testname", default='test.csv', help="input a testing data filename")
...
cmdparser.add_option("-r", "--resume", action="store_true", dest="resume", default=False, help="continue ?")
...
if resume:
    params=tl.files.load_npz(name='cnn_82.npz')
    tl.files.assign_params(sess, params, network)
network.print_layers()
#訓練
'''
tl.utils.fit(sess, network, train_op, cost, x_train, y_train, x, y_,
            acc=acc, batch_size=Batch, n_epoch=int(options.epoch), print_freq=2,X_val=x_test, y_val=y_test,
            save_name='cnn_83',save_acc=0.83,eval_train=True,tensorboard=options.graph, tensorboard_epoch_freq=2)
'''
# 评估模型
